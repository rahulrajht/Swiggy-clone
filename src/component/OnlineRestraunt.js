    ...
    return (
        <div className="mt-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 w-full mt-8">
                {loading ? <p>Loading ...</p> : onlineRestraunt?.map((item) => (
                    <Link to={item && `${`restraunt/${item.info.id}`}  ${item.info.name} ${item.info.locality} ${item.info.areaName}`.split(" ").join("-").toLowerCase()} className="w-[20rem] block">
